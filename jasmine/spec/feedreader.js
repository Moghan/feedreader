...
$(function() {
	...
	describe('RSS Feeds', function() {
         ...
         */
		it('feed URLs are defined and valid', () => {
			allFeeds.forEach((feed) => {
				expect(feed.url).toBeDefined();

                // Found the expression in the discussion forum.
	...
	describe('Initial Entries', () => {
         ...
         */
		it('loadFeed() completes and at least one element is in container', (done) => {
			var htmlElement_Entries = $('.entry');
	...
	describe('New Feed Selection', () => {
		...
		beforeEach( (done) => {
			if(allFeeds.length > 1) {
				loadFeed(0, () => {
					// Alternative content = $('.feed').html();
					oldContent = $('.entry h2')[0].textContent;
					loadFeed(1, done);
				});
			}
		});
